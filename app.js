  ...
  appData: {
    appId: config.service.appId,
      ...
      */
      ...
      getSubjectGates(page,subjectid) {
            var user = this.getUserData();
            if (!user)
                  return null;
            ...
            var gates = {}, degrees = {}, services = {};
            services = user.services;

            if (!(subjectid in services))
                  return null;

            var subject = services[subjectid];     
            for (var v in subject.degrees) {
                  var degree = subject.degrees[v];
                  if (degree.img.indexOf("/") === -1)
                        degree.img = config.service.host + "/static/media/gate_img/" + degree.img;
            }
      ...
      getAllSubjects(page) {
            ...
            var services = user.services;
            var subjects = [];
            var degree = user.status===2?1:0;

            for(var v in services){
                  var service = services[v];
                  if (service.id != 0 && degree === service.degree){
                        console.log("v==>",v,service);
                        if (service.img && service.img.indexOf("/") === -1)
                              service.img = config.service.host + "/static/media/subject_img/" + service.img;
                        subjects.push(service);
                  }                        
            }
            
            page.setData({
                  subjects: subjects.sort(util.compare("id")),
            }) 
      },
